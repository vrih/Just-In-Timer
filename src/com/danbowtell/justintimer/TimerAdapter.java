...
class TimerAdapter extends ArrayAdapter<Timer> {
	...
	public View getView(int position, View convertView, ViewGroup parent){
		...
		if(o != null){
			...
			if(o.getPaused()){
					setColors(BLACK,v,YELLOW,fn.getDisplay(o.getRemaining()),name,time);
		    } else if(o.isRunning()){
		    		setColors(BLACK,v,GREEN,fn.getDisplay(o.getRemaining()),name,time);
		    } else 	if(o.isEnded()){
		    	setColors(BLACK,v,RED, fn.getDisplay((long) 0),name,time);
			} else if(!o.getPaused() && !o.isRunning()){
					setColors(GREY,v,BLACK,fn.getDisplay(o.getTimerTime()),name,time);
			}
		}
		return v;
	}
	
	private void setColors(int id, View v, int bgcol, String text, TextView name, TextView time){
		if( id != 0 && bgcol != 0){
